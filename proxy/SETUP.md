...
curl -X POST https://judal-proxy.<계정>.workers.dev/notice `
  ...
  -d '{\"notice_no\":\"R26BK01471632\"}'
```

`{ "notice": { ... presmptPrce ... } }` 가 오면 정상.
`{ "error": ... }` 면 SETUP.md 1~3 단계 키/토큰 재확인.

> 엔드포인트·필드 매핑은 실제 조달청 응답으로 검증·보정 완료됨.
> (입찰공고: getBidPblancListInfoCnstwk / 개찰결과:
>  getOpengResultListInfoCnstwkPPSSrch — 낙찰자 투찰률 실분포)
> 공고번호는 영문숫자(예: R26BK01471632) 형식이며 가공 없이 그대로 사용.
